...
//        Member fetchFirst = jpaQueryFactory.selectFrom(member).fetchFirst();
    ...
    /**
     * 회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조인하고 회원은 모두 조회해라.
    ...
    public void join_on_no_relation() {
        ...
        List<Tuple> result = jpaQueryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team).on(member.username.eq(team.name)) // leftJoin(team) 을 쓰면 id로 조인하지 않고, on 절로만 join.
                .fetch();
    ...
    /**
     * Tuple은 Querydsl 종속이기 때문에 dao 안에서만 사용하고, 로직에서는 dto로 변환해서 사용하는 것이 좋다.
     */
    @Test
    ...
    public void findDtoByJPQL() {
        // 생성자 방식만 지원하고 패키지명까지 적어야하기 때문에 복잡하다.
        List<MemberDto> resultList = em.createQuery("select new study.querydsl.entity.MemberDto(m.username, m.age) from Member m", MemberDto.class)
                .getResultList();
    ...
    public void findDtoByField() {
        // setter로 주입하는게 아니라 필드에 값을 직접 때려박는다.
        List<MemberDto> fetch = jpaQueryFactory
    ...
    public void bulkUpdate() {
        // bulk 쿼리 수행 시 DB에는 업데이트되지만 영속성 컨텍스트는 유지된다.
        // 뒤에 select 수행하면 영속성 컨텍스트가 우선순위를 갖는다.
